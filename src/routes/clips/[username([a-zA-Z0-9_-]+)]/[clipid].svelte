<script context="module">
  export async function preload(page, { user }) {
    const { username, clipid } = page.params;

    try {
      const user_response = await this.fetch(`api/v1/users/${username}`);
      const { Items, message } = await user_response.json();
      ...
      if (
        ...
        this.error(
          user_response.status === 200 ? 404 : user_response.status,
          message || "Could not get user."
        );

      const { id: userid, name } = Items[0];

      const socials_response = await this.fetch(
        `api/v1/users/${userid}?includeSocials`
      );
      const { Socials } = await socials_response.json();

      if (!userid)
        this.error(user_response.status, message || "Could not get user.");
